...
|---|---|---|---|---|---|---|---|
| B1 | Critical | Rent entered as USD $800 silently saved as GHS 800; wrong figure spread to vacancy, forecast, P&L | **Claim disproved, worse problem found.** DB holds `Main House / 800.0000 / USD` — the currency saved correctly. But Settings → Currency invites the landlord to set a USD→GHS rate (defaults to 15) and that rate is applied nowhere. | `CashFlowServiceImpl.java:122-129` sums `unit.getRent()` with no reference to `unit.getCurrency()`. `utils/currency.ts` exports `toGhs()` and `formatDual()` — **zero call sites in the entire frontend**. | **Re-diagnosed, and it is not what it looked like.** The save is correct — both dollar-priced units are stored as `USD`. The units list hardcoded `₵` over the value, and the edit dialog was never handed the stored currency, so it showed "Rent (GHS)" and the next save really would have overwritten USD with GHS. Three visits of "the save throws the currency away" were a display bug plus a latent one. | `FIXED` | `375c31e` — list and edit form both use the stored currency; browser-verified as `$800.00` beside `GH₵850.00`. **Still open as a product decision:** what rate converts a dollar rent into a cedi total (vacancy sum, forecast, P&L). M6 depends on that answer, not on this fix |
...
| B8 | High | 15-minute session; refresh refused as device mismatch revokes the whole session with nothing on screen | **Confirmed from the server.** Login response carries `expiresIn: 900` (15 min). The log for the reporter's session shows `SECURITY_EVENT: action=REFRESH_TOKEN_DEVICE_MISMATCH, family=…, result=FAMILY_REVOKED`, immediately after `JWT expired`. Nothing was shown to him; the page kept its furniture and swallowed clicks. | A refresh that fails device matching revokes the entire token family. Correct as a security control; the defect is that the client has no handler that tells the user they have been signed out. | **Already built; closed on evidence.** An involuntary logout carries a reason, redirects to `/login?reason=…&redirectTo=…`, and the login page renders the reason and returns the user to where they were. Verified live: the banner reads "Session expired. Please login again." with `redirectTo=/wallet` preserved. The landlord saw this himself in Chapter 3 — his "no message at all" note was written in Chapter 2, before he hit it. | `FIXED` | no code needed — the mechanism was already in place and works |
...
| M17 | Low | Units must be added one at a time; the form forgets property/type/rent between each | **Confirmed.** | `view/AddUnitDialog.tsx:105-118` — on every open for a new unit the form is reset to blank defaults (`unitNo: ''`, `type: '1br'`, `rent: ''`). No bulk-add path exists. Eight rooms means retyping type and rent eight times. | Low | `FIXED` | `171df55` (property-page dialog) + `e8b6abb` (All Unit dialog, plus Status defaulting to Available). ⚠️ Re-opened by the 2026-08-25 re-test: the first commit fixed only one of TWO add-unit dialogs and I closed the row on the one I happened to open. The landlord used the other. Each dialog now has its own test. |
...
| N7 | Record Advance capped at 12 months while the system holds a 24-month advance | **CONFIRMED, and worse than reported.** `limitsService.validate` is called only from `initiateGatewayPayment`; `create()` never validates. So the backend does not cap the manual path at all — Adjoa's 24-month advance went through it — while the form applied the cap to both modes. A landlord could not record the two-year advance he had just taken in cash. | `FIXED` | `89f7d02` — cap scoped to the gateway/request mode; tests pin both halves |
| N8 | Expense cannot be linked to the maintenance request it came from | **CONFIRMED as a visibility gap, not a missing link.** `MaintenanceExpenseSync` already writes the expense on close, idempotently, keyed on `maintenance_request_id` (V161). Nothing on the request said so, so mid-repair the landlord sees no expense and is about to enter it twice. | `FIXED` | `89f7d02` — the request states where the cost goes and whether it has gone |
| N9 | Receipt still fails ("Failed to open receipt") — third sighting | **CONFIRMED, client-side only.** The endpoint returns a valid receipt (200, 3,638 bytes, `RCPT-20260825-46E1D7`). The failure is a blocked pop-up, reported as "Failed to open receipt" — untrue and unactionable, and both callers discarded the helper's real message. | `FIXED` | `89f7d02` — blocked pop-up now falls back to a download; callers surface the reason |
| N10 | Units list prints `Self_contained` with the underscore | **CONFIRMED.** The cell printed `row.original.type` raw under a `capitalize` class, so `self_contained` rendered as "Self_contained". | `FIXED` | `89f7d02` — reference label, with a fallback that de-underscores anything the list has not got |
