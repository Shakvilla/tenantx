...
const selectPropertyAndUnit = async () => {
...
describe('TenantHomeStep', () => {
  beforeEach(() => {
    ...
    vi.mocked(lookupOccupants).mockResolvedValue([])
  })
  ...
  it('blocks Continue and shows an error for an invalid email format', async () => {
    setup()
    await selectPropertyAndUnit()
    fireEvent.change(screen.getByLabelText(/first name/i), { target: { value: 'Kwabena' } })
    fireEvent.change(screen.getByLabelText(/last name/i), { target: { value: 'Owusu' } })
  ...
  it('blocks Continue and shows an error for an invalid phone number', async () => {
    setup()
    await selectPropertyAndUnit()
    fireEvent.change(screen.getByLabelText(/first name/i), { target: { value: 'Kwabena' } })
    fireEvent.change(screen.getByLabelText(/last name/i), { target: { value: 'Owusu' } })
    ...
    fireEvent.click(screen.getByRole('button', { name: /^continue$/i }))
    expect(await screen.findByText(/valid ghana phone number/i)).toBeInTheDocument()
    expect(createOccupant).not.toHaveBeenCalled()
  })
  ...
  it('rejects names containing numbers', async () => {
    setup()
    await selectPropertyAndUnit()
    fireEvent.change(screen.getByLabelText(/first name/i), { target: { value: 'Kwabena2' } })
    fireEvent.change(screen.getByLabelText(/last name/i), { target: { value: 'Owusu' } })
    fireEvent.change(screen.getByLabelText(/email/i), { target: { value: 'k@example.com' } })
    fireEvent.change(screen.getByLabelText(/phone/i), { target: { value: '0244123456' } })
    fireEvent.click(screen.getByRole('button', { name: /^continue$/i }))
  ...
  it('auto-searches on email blur and shows a confirm panel for an exact email match', async () => {
    vi.mocked(lookupOccupants).mockResolvedValue([
      {
        id: 'occ-existing',
        firstName: 'Irfan',
    ...
    await waitFor(() => expect(lookupOccupants).toHaveBeenCalledWith('t1', 'x@e.com', ''))
    expect(await screen.findByText(/already belongs to an occupant/i)).toBeInTheDocument()
    expect(createOccupant).not.toHaveBeenCalled()

    fireEvent.click(screen.getByRole('button', { name: /assign irfan to unit 100/i }))

    expect(onComplete).toHaveBeenCalledWith(
      expect.objectContaining({
        ids: expect.objectContaining({ occupantId: 'occ-existing', unitId: 'u1', unitNo: '100' }),
  ...
  it("'Use a different email' clears the confirm panel and re-enables a fresh search", async () => {
    vi.mocked(lookupOccupants).mockResolvedValue([
      {
        id: 'occ-existing',
        firstName: 'Irfan',
    ...
    expect(screen.queryByText(/already belongs to an occupant/i)).not.toBeInTheDocument()
    expect(screen.getByRole('button', { name: /^continue$/i })).toBeInTheDocument()
    expect(screen.getByLabelText(/email/i)).toHaveValue('')
  })

  it('shows an identity-confirm panel on phone match, reuses on "yes", and allows "new tenant" to proceed', async () => {
    vi.mocked(lookupOccupants).mockResolvedValue([
      {
        id: 'occ-phone',
        firstName: 'Ama',
    ...
    fireEvent.change(screen.getByLabelText(/phone/i), { target: { value: '0244999999' } })
    fireEvent.blur(screen.getByLabelText(/phone/i))

    expect(await screen.findByText(/is it the same person/i)).toBeInTheDocument()

    fireEvent.click(screen.getByRole('button', { name: /yes, it's them/i }))

    expect(onComplete).toHaveBeenCalledWith(
      expect.objectContaining({
        ids: expect.objectContaining({ occupantId: 'occ-phone', unitId: 'u1', unitNo: '100' }),
        occupantName: 'Ama Boateng'
      })
    )
    expect(createOccupant).not.toHaveBeenCalled()
  })

  it('dismisses the phone-match panel on "new tenant" and still creates a new occupant on Continue', async () => {
    vi.mocked(lookupOccupants).mockResolvedValue([
      {
        id: 'occ-phone',
        firstName: 'Ama',
    ...
    fireEvent.click(screen.getByRole('button', { name: /no, this is a new tenant/i }))

    expect(screen.queryByText(/is it the same person/i)).not.toBeInTheDocument()

    fireEvent.change(screen.getByLabelText(/first name/i), { target: { value: 'Nana' } })
    fireEvent.change(screen.getByLabelText(/last name/i), { target: { value: 'Yeboah' } })

    // Continue re-checks by email only — this is a phone-only coincidence, not a blocking duplicate.
    fireEvent.click(screen.getByRole('button', { name: /^continue$/i }))
