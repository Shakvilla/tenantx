...
/** A FLAT-priced source: single entryPrice applied to every billable unit. */
 ...
 * amount = billableUnits × pricePerUnit (now entryPrice for FLAT plans). If this
 * drifts from that, the page quotes a price the invoice will contradict, which is
...
describe('calculateMonthlyCharge', () => {
  it('charges only the units above the free allowance', () => {
    ...
    const charge = calculateMonthlyCharge(9, flat(30), 5)
  ...
  it('charges nothing while the portfolio is inside the allowance', () => {
    const charge = calculateMonthlyCharge(5, flat(30), 5)
  ...
  it('bills every unit when there is no allowance', () => {
    expect(calculateMonthlyCharge(9, flat(30), null).monthlyTotal).toBe(270)
    expect(calculateMonthlyCharge(9, flat(30), 0).monthlyTotal).toBe(270)
  })

  it('does not go negative or fractional on odd input', () => {
    expect(calculateMonthlyCharge(-3, flat(30), 5).monthlyTotal).toBe(0)
    expect(calculateMonthlyCharge(2.7, flat(30), 0).billableUnits).toBe(2)
  })
})

describe('describeMonthlyCharge', () => {
  it('shows the subtraction, so the landlord can check it', () => {
    const text = describeMonthlyCharge(calculateMonthlyCharge(9, flat(30), 5), ghs)
  ...
  it('says plainly when nothing is owed yet', () => {
    expect(describeMonthlyCharge(calculateMonthlyCharge(4, flat(30), 5), ghs)).toContain('GH₵ 0.00')
    expect(describeMonthlyCharge(calculateMonthlyCharge(0, flat(30), 5), ghs)).toMatch(/nothing to pay/i)
  })
})
